...
for i in $(seq 1 180); do : "$i"; status=$(docker inspect -f '{{if .State.Health}}{{.State.Health.Status}}{{else}}unknown{{end}}' "$app") || status=unknown; [ "$status" = healthy ] && { printf '%s\n' '[xymedia] 阶段：迁移完成，应用健康状态为 healthy。'; trap cleanup 0; trap - HUP INT TERM; exit 0; }; sleep 1; done
